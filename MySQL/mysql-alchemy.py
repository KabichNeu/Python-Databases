...
class Task(Base):
    ...
    project = realtionship("Project")
    
    def __repr__(self):
        return "<Task(task='{0}')>".format(
            self.description
        )
...
organize_closet_project = Project(title = 'Organize closet', description = 'Organize closet by color ')
...
our_tasks = session.query(Task).filter_by(title = 'Organize closet').first()
print(our_tasks)